// Create a temporary endpoint to fix the PostgreSQL function
...
// Create a new pool with explicit password handling
const pool = new Pool({
...
app.post('/fix-function', async (req, res) => {
  let client;
  try {
    ...
    res.json({ 
      success: true, 
      message: 'PostgreSQL function fixed successfully!' 
    });
...
const PORT = 5003;
app.listen(PORT, () => {
  console.log(`🔧 Temporary fix server running on port ${PORT}`);
  console.log(`📡 Run: curl -X POST http://localhost:${PORT}/fix-function`);
});
