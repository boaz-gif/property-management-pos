// Use the existing backend database connection to fix the function
const Database = require('../src/utils/database');

async function fixFunctionViaBackend() {
  try {
    console.log('Fixing PostgreSQL function via backend connection...');
    
    // Drop the existing function
    console.log('1. Dropping existing function...');
    await Database.query('DROP FUNCTION IF EXISTS calculate_admin_dashboard_metrics(p_admin_id INTEGER, p_date DATE)');
    console.log('✅ Existing function dropped');
    
    // Create the fixed function
    console.log('2. Creating fixed function with DATE_PART...');
    const createFunctionQuery = `
      ...
      BEGIN
          ...
          -- Get maintenance metrics with DATE_PART instead of EXTRACT
          SELECT 
    ...
    console.log('✅ Fixed function created successfully');
    
    // Test the function
    console.log('3. Testing the function...');
    await Database.query('SELECT calculate_admin_dashboard_metrics(1, CURRENT_DATE)');
...
// Run the fix
fixFunctionViaBackend()
  .then(() => {
    console.log('✅ All done! The function is now properly fixed.');
    process.exit(0);
  })
  .catch((error) => {
