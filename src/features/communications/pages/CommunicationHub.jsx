...
import documentApi from '../../tenants/api/documentApi';
...
const CommunicationHub = () => {
  ...
  const refreshConversations = async () => {
    setLoadingList(true);
    try {
      const res = await communicationApi.listConversations();
      ...
      if (!ensuredDefaultConversationsRef.current) {
        ...
        if (role === 'tenant') {
          ...
          if (!hasCommunity || !hasAdminDm) {
            ...
            await Promise.allSettled([communicationApi.ensureCommunity(), communicationApi.ensureAdminDm()]);
            const reloaded = await communicationApi.listConversations();
  ...
  const loadThread = async (id) => {
    ...
    try {
      const [convRes, msgRes] = await Promise.all([
        communicationApi.getConversation(id),
        communicationApi.listMessages(id, { limit: 100 })
      ]);
      ...
      if (msgData.length > 0) {
        ...
        await communicationApi.markRead(id, { message_id: lastMessageId });
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const res = await communicationApi.sendMessage(resolvedConversationId, { content, attachments });
      ...
      await communicationApi.markRead(resolvedConversationId, { message_id: message?.id });
    } finally {
